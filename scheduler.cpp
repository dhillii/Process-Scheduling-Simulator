...
void PSJF_Scheduler::findWaitingTime(Process proc[], int n, int wt[]){
    int rt[n]; 
...
void PSJF_Scheduler::findTurnAroundTime(Process proc[], int n, int wt[], int tat[]){
    // calculating turnaround time by adding 
    // bt[i] + wt[i] 
    for (int i = 0; i < n; i++) 
        tat[i] = proc[i].bt + wt[i]; 
...
void PSJF_Scheduler::findavgTime(Process proc[], int n){
